...
const Callout = Node.create({
  ...
  renderHTML({ node, HTMLAttributes }: any) {
    return [
      ...
      ["span", { class: "callout-emoji select-none", contenteditable: "false" }, node.attrs.emoji],
...
export default function TipTapEditor({
  ...
  const [aiActionBar, setAiActionBar] = useState<{
    startPos: number; endPos: number; commandType: string
    prompt: string; coords: { top: number; left: number }; originalText?: string
  } | null>(null)
        ...
        if (node.type.name === "callout") {
          ...
          if (target.classList.contains("callout-emoji") || target.closest?.(".callout-emoji")) {
            ...
            if (newEmoji !== null) {
              ...
              view.dispatch(view.state.tr.setNodeMarkup(nodePos, undefined, { ...node.attrs, emoji: trimmed || "💡" }))
            }
            return true
          }
        }
  ...
  const snippetCommands = snippets.map((s) => ({
    id: s.id, title: s.title, desc: `Salin aset: "${s.title}"`,
    icon: IconSparkles, content: s.content,
  }))

  // Slash menu hanya tampilkan block-type commands (bukan inline format)
  const blockFormatIds = ["h1","h2","h3","bullet","number","todo","quote","callout","divider","code","table"]
  const slashFormatCommands = formatCommands.filter(c => blockFormatIds.includes(c.id))

  const allItems: EditorCommandItem[] = [
    ...slashFormatCommands.map((item) => ({ ...item, type: "format" as const })),
  ...
  const handleAiToolbarClick = () => {
    ...
    setAiPromptPlaceholder("Contoh: tulis hook pembuka yang menarik perhatian...")
    setIsAiPromptActive(true)
    try {
      const coords = editor.view.coordsAtPos(editor.state.selection.from)
      setAiPromptCoords({ top: coords.top, bottom: coords.bottom, left: coords.left })
  ...
  const streamAiGenerate = async (commandType: string, userPrompt: string, customContext?: string) => {
    ...
    try {
      const response = await fetch("/api/ai", {
        ...
        body: JSON.stringify({ prompt: userPrompt, context: contextToUse, commandType, draftMeta }),
      })
      ...
      try {
        ...
        const scrollContainer = containerRef.current?.querySelector(".simple-editor-content")
        if (scrollContainer) {
        ...
        if (containerRect) {
          ...
          const padding = 16
          barLeft = Math.max(containerRect.left + padding, Math.min(coords.left, containerRect.right - barWidth - padding))
        }
        setAiActionBar({
          startPos, endPos: finalEndPos, commandType, prompt: userPrompt,
          coords: { top: barTop, left: barLeft },
          originalText: isSelection ? originalText : undefined,
        })
      } catch {
        setAiActionBar({ startPos, endPos: finalEndPos, commandType, prompt: userPrompt, coords: { top: 200, left: 100 }, originalText: isSelection ? originalText : undefined })
      }
    } catch (e) {
  ...
  const handleAiAccept = () => { setAiActionBar(null); editor?.commands.focus() }

  const handleAiDiscard = () => {
    if (!aiActionBar || !editor) return
    const { startPos, endPos, originalText } = aiActionBar
    if (originalText) {
      editor.chain().focus().deleteRange({ from: startPos, to: endPos }).insertContentAt(startPos, originalText).run()
    } else {
  ...
  useEffect(() => {
    triggerExecuteRef.current = () => {
      if (filteredItems[selectedIndexRef.current]) executeCommand(filteredItems[selectedIndexRef.current])
    }
  }, [filteredItems])
